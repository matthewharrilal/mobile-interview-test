...
`Cmd+R` to build and run. `Cmd+U` to run the test suite. iOS 17+ Simulator. The `.xcodeproj` is committed alongside its `project.yml`, so reviewers don't need XcodeGen.
...
The morph is the iOS 18 zoom transition with an iOS 17 fallback path. The parallax hero stretches via a damped rubber-band response driven by `CADisplayLink` at 120 Hz on ProMotion. If video files aren't present yet, run `maestro test .maestro/01-happy-path.yaml` for the morph or `.maestro/15-swipe-down-dismiss-rubber-band.yaml` for the parallax to see them live.
...
The layer is fully typed end-to-end. Raw HTTP method strings, header names, content types, and log event names are all small enums with helper extensions on `URLRequest`. The POST body for the hotels endpoint is a typed `Encodable` struct, not `[String: Any]` + `JSONSerialization`. Array decoding is lossy by default — one malformed `Place` or `Hotel` row drops to nil without nuking the whole response, surfaced via a small `FailableDecodable<T>` wrapper. Decode errors are wrapped in a domain error variant so the view-model boundary sees a uniform error taxonomy that knows how to classify retryability.

### Navigation

`NavigationStack(path:)` with a value-based `AppDestination` enum. The path is bound to `state.path` via a binding whose setter dispatches a `.pathChanged` intent — pushes (intent-triggered) and pops (swipe-back) both flow through the reducer, preserving the unidirectional invariant.

The detail screen uses iOS 18's matched-transition-source + zoom navigation transition as the primary morph. iOS 17 falls back to `matchedGeometryEffect` + a `ZStack` overlay path. Both paths compile against the iOS 17 SDK via an `@available`-gated modifier.

### Why MVI, not TCA or MVVM

For two screens, TCA's framework dependency and learning surface didn't justify the cost. MVI captures the same essentials — unidirectional flow, explicit intent enum, replayable transitions, sync reducer, structured concurrency for side effects — without the third-party dependency. MVVM with `@Observable` would also work; I prefer the explicit intent enum because every state mutation becomes discoverable in one place (the reducer's `switch`), which I value for testability and onboarding.
...
| **Concurrency** | `async/await` + `Task` + `ContinuousClock` | Injectable clock makes the 500ms debounce testable without timing flakes. No Combine for a single operator. |
| **Networking** | `URLSession` wrapped in a `Sendable` transport seam + one orchestration helper | Two endpoints don't justify Alamofire. The helper centralizes everything cross-cutting so feature clients stay tiny. |
| **State management** | Hand-rolled MVI with `@Observable` | Unidirectional flow, explicit intent enum, sync reducer. See **Why MVI** above. |
| **Dependency Injection** | Manual constructor injection + a composition-root value | View models take their specific client; views read the shared graph through the environment. One file to read for the full dependency graph. See [ADR-008](docs/ADRs.md). |
| **Client shape** | Function-style: `Sendable struct` of `@Sendable` closures | Swap-for-test is one line. Sendable falls out for free. See [ADR-003](docs/ADRs.md) for the protocol-vs-function-style steelman + the migration sketch past N=4 endpoints. |
| **Navigation** | `NavigationStack(path:)` value-based with `AppDestination` enum | Type-safe deep-link surface; new screens add an enum case. Path mutations flow through the reducer. |
| **Data modeling** | `Codable` with explicit `CodingKeys` (not `.convertFromSnakeCase`) | The wire has mixed-case keys; auto-conversion gets it wrong. Per-type keys are explicit. |
| **Defensive decoding** | Lossy array decode by default via a small `FailableDecodable<T>` wrapper | One malformed row from the staging API drops to nil rather than nuking the whole response. |
| **Image caching** | Kingfisher 8.x via a thin wrapper, plus a custom editorial-grade processor | 100 MB / ~1 GB memory + disk LRU out of the box. The processor (Core Image + Metal/MPS fallback) runs once per URL and is cached against the processor identifier — never recomputed on scroll. Two cache-warming windows: listings grid on `.loaded`, detail carousel on card tap. |
| **Minimum iOS** | 17.0 | Spec floor is 16; lifted to 17 for `@Observable`, `ContentUnavailableView`, value-based `NavigationStack(path:)`, and `ContinuousClock`. See [ADR-002](docs/ADRs.md). |
| **Localization** | `String(localized:)` with `defaultValue:` everywhere | ~30 calls in a single `Strings` namespace. A real `.xcstrings` catalog drops in cleanly; an `es.lproj` stub exists demonstrating resolution. |
...
|---|---|---|
| Status-bar bridge | `UIHostingController` exposing `preferredStatusBarStyle` | iOS 17 has no native SwiftUI API to coordinate the system status bar with a morph. Gated to iOS < 18. |
| Transition adapters (4 files) | `UIPresentationController` + `UIPercentDrivenInteractiveTransition` + animated transitioning | iOS 17 fallback morph scaffolding. Belt-and-suspenders for the OS-version split. |
| Filter-chip pulse | `UIViewRepresentable` over `CALayer` + `CABasicAnimation` | Frame-precise, vBlank-aligned pulse that's distinct from SwiftUI's transaction system. |
| Hairline tokens | `UIScreen.main.scale` | Physical-pixel-crisp hairline width. SwiftUI has no universally-accessible `displayScale` for a token file. |
| Editorial-grade processor | `UIImage` + Core Image + Metal/MPS | Kingfisher's processor protocol takes `UIImage`. Metal path runs the grade on cache miss; Core Image is the CPU fallback. |
| Drag-throw spring | `CADisplayLink` | 120 Hz spring integration on ProMotion. Documented as a deliberate supplemental driver in [ADR-007](docs/ADRs.md). |
...
| | |
|---|---|
| **State** | Six statuses including a dedicated `failedNullCoords` case for a real-API edge case |
| **Key choice** | 500 ms debounce via injected `ContinuousClock` — testable, no sleep-flakes |
| **Edge case** | Brooklyn FL has null lat/lng in the staging API; the VM surfaces a "search a nearby city" CTA instead of a "Try Again" loop that would re-pick the same offending place. The transport layer also fails loudly on null coords as belt-and-suspenders. |
| **Maestro find** | Landscape keyboard occluded the retry CTA in failed/empty states — fixed via auto-dismiss of the keyboard on status change. |
| **Tests** | 11 reducer tests + 21 spec-compliance + 6 direct live-client tests |

### Hotel listings

| | |
|---|---|
| **State** | Nested presentation layer (`browsing` / `detailExpanded`), status enum, loaded payload with `fetchedAt` and pre-built sections, filter taxonomy |
| **Section pipeline** | Pure section builder produces Top picks (≤5 by rating), Within walking distance (≤1.5 mi), Best value (cheapest 5), and an All fallback. Cached on the loaded payload; rebuilt only on hotels / filter mutation. |
| **Filters** | All / Pool / Spa / Adults / Pets / Wellness — string-match heuristic against the hotel's product name and primary vibe |
| **Cache warming** | Two windows: first 30 cards on `.loaded`, next 4 carousel images on card tap |
| **Spec deviation** | Sectioned horizontal carousels rather than the spec's vertical list. Reasoning lives below in **§12** + [ADR-009](docs/ADRs.md). |
| **Tests** | 5 reducer + 11 section-pipeline + 9 direct live-client tests |

### Hotel detail

| | |
|---|---|
| **Morph** | iOS 18: matched-transition-source + zoom navigation transition. iOS 17: `matchedGeometryEffect` + ZStack overlay. Gated so both paths compile. |
| **Drag-throw dismiss** | `CADisplayLink`-driven spring integrator running at the native display refresh — 120 Hz on ProMotion. Rubber-band response with snap-back. |
| **Parallax hero** | Sticky header stretches on pull-down with a damped scale; behavior pinned by a dedicated Maestro flow. |
| **Image carousel** | Peek + mid-snap, processor-aware caching matched to the upstream Kingfisher prefetch. |
...
### Unit tests (120 across 12 files)

Coverage spans: reducer behavior per feature, spec-pinning (500 ms debounce timing, dismiss-no-refetch regression, scene-phase staleness, presentation transitions), URLProtocol-stubbed transport, every URL endpoint vs the spec verbatim (including percent-encoding + CJK), every `URLError` mapping, direct coverage of the live client factories (so the transport orchestration helper is verified end-to-end, not just through the view-model seam), the section-builder pipeline, the logging surface, and real-fixture decoding plus adversarial inputs for both `Place` and `Hotel`.

### Maestro flows (81 yaml files in `.maestro/`)
...
| Animation audits | 10 | Stretchy hero, transitions, morph spring envelope, swipe-down dismiss (rubber-band + 200 pt commit), search-pill blur, peek carousel mid-snap, filter chip mid-transition, hero pulldown, content fade-in |
| Filter chips | 6 | Spa, Adults, Pets, Wellness, empty result, reset |
| Retry recoveries | 2 | Search-retry-failed, hotels-retry-failed |
| Loading skeletons | 2 | Search, hotels |
| Dark mode | 10 | All states × both screens × detail expanded × empty filter |
| XXL Dynamic Type | 5 | All states |
| Landscape coverage | 7 | Search empty/failed/null-coords, hotels empty/failed, detail expanded, empty filter |
| German locale | 3 | Long-string truncation verification |
| Orphan-code regression | 4 | Verifies deleted UI surfaces are not mounted |
| Edge cases | 6 | Pull-to-refresh absence (pinned), rapid-type-then-clear, debounce-window collision, offline (search + hotels), slow 3G image loading |
...
- **Landscape keyboard occluded the retry CTA** in compact-height layouts. Fixed via `.scrollDismissesKeyboard(.immediately)` + status-change auto-dismissal.
- **iOS 26 + Maestro 2.5.1** — the Maestro driver can't walk the iOS 26 SwiftUI accessibility tree (`assertVisible` on body text fails while the app renders identically on iOS 18). Maestro work pinned to iOS 18 sim.
- **Null-coordinate dead-end** — selecting an alias-only place would otherwise have looped the user into a re-failure with no recovery affordance. Pinned by paired happy-path and null-coords flows.

### Snapshot testing

Not implemented in this submission. Listed in **§14** below — `swift-snapshot-testing` for the state × portrait/landscape × light/dark matrix would close the visual-regression gap that Maestro's text-assertion model can't directly cover.
...
| **Maestro smoke** | Every push + PR (after unit tests) | ~5–10 min | Five critical flows: happy path, landscape, search empty, hotels empty, search-failed-retry |
| **Maestro full** | Weekly schedule + manual dispatch | ~30–40 min | All 81 Maestro flows |

xcresult bundles and Maestro screenshots upload as workflow artifacts on failure. Concurrency control cancels in-progress runs when a new commit lands on the same ref. Maestro in CI uses the same fail-on-missed-assertion model as local runs — flows hit the real staging API, so an outage will fail CI (accepted trade-off for high-fidelity coverage; flagged in the workflow's top comment).
...
| **006** | Scene-phase staleness policy — 5 minutes | Balances "briefly switched to Messages" (no refetch) vs "came back hours later" (real stale-data risk). |
| **007** | Value-bound animation, not `PhaseAnimator` | iOS 18 zoom transition re-instantiates the destination view, desyncing the phase index from a preserved `@State` trigger. Value-binding has no internal state to desync. |
...
13. **Video recording workaround** — QuickTime or `ffmpeg avfoundation` with Screen Recording permission so the demo videos ship as real motion in `docs/media/`.
